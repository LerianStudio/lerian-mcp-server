...
import { ValidationResult, AuditLog, RateLimitEntry } from '../types/index.js';
 ...
 */
export function isLocalConnection(request?: Request): boolean {
 ...
 */
export function sanitizeSensitiveData(data: any, depth: number = 0): any {
 ...
 */
export function createAuditLog(entry: Record<string, any>): AuditLog | null {
  if (!SECURITY_CONFIG.auditLoggingEnabled) {
 ...
 */
export function validateInput(schema: z.ZodSchema, data: any): ValidationResult {
  try {
 ...
 */
function checkForInjectionPatterns(data: any, depth: number = 0): void {
  if (depth > 10) return;
 ...
 */
export function readSecureConfig(configPath: string): any {
  try {
 ...
 */
function validateConfigSecurity(config: any): void {
 ...
 */
export function createSecureEnvironment(): void {
  // Drop unnecessary environment variables
  const allowedEnvVars: string[] = [
    'PATH',
    'HOME',
    'USER',
    'LANG',
